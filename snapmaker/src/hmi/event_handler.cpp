...
void GCodeQueue::get_hmi_commands() {
  ...
  while (length < BUFSIZE && hmi_commands_in_queue > 0) {
    // fetch from buffer queue
    Screen_send_ok_opcode[index_w] =
        hmi_send_opcode_queue[hmi_cmd_queue_index_r];
    CommandLine[index_w] =
        hmi_commandline_queue[hmi_cmd_queue_index_r];
    _enqueue(hmi_command_queue[hmi_cmd_queue_index_r]);
 ...
 */
bool GCodeQueue::ok_to_HMI() {
  return Screen_send_ok_opcode[index_r];
}
