...
PowerLossRecovery pl_recovery;
  ...
  extern float fast_move_feedrate;
  extern float print_move_feedrate;
#endif
...
int PowerLossRecovery::SaveEnv(void) {
  ...
  uint8_t *pBuff;
	...
	cur_data_.axis_relative = gcode.axis_relative;

  cur_data_.toolhead = ModuleBase::toolhead();

  cur_data_.PrintFeedRate = print_move_feedrate;
  cur_data_.TravelFeedRate = fast_move_feedrate;
...
ErrCode PowerLossRecovery::ResumeWork() {
	...
	print_move_feedrate = pre_data_.PrintFeedRate;
	fast_move_feedrate = pre_data_.TravelFeedRate;

	gcode.axis_relative = pre_data_.axis_relative;
