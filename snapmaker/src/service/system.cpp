...
ErrCode SystemService::PauseTrigger(TriggerSource type)
{
  ...
  if (xEventGroupGetBits(sm2_handle->event_group) & EVENT_GROUP_WAIT_FOR_HEATING) {
    ...
    pl_recovery.SaveCmdLine(queue.command_line());
...
ErrCode SystemService::ResumeProcess() {
  ...
  // restore speed
  fast_move_feedrate = pl_recovery.cur_data_.TravelFeedRate;
  print_move_feedrate = pl_recovery.cur_data_.PrintFeedRate;
...
/**
 * Check if exceptions happened, this should be called in idle()[MarlinCore.cpp]
